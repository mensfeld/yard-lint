## Unreleased
- **[Fix]** `Tags/MissingYield` no longer attributes a nested method's `yield` to the enclosing method. The check scanned the entire method source, so `def outer; def inner; yield; end; end` flagged `#outer` even though only `#inner` yields. Detection now uses Ripper to find a `yield` that belongs to the method's own body, descending past nested `def`/`def self.` definitions. All previously detected cases (yield with arguments, `yield self`, `return yield`, yields inside blocks, etc.) are unchanged; a line-based scan is kept as a fallback for sources Ripper cannot parse.
