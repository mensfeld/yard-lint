## Unreleased
- **[Feature]** A category-level `Severity` (e.g. `Documentation: { Severity: convention }`) is now honored for every validator in that department, matching how category-level `Enabled` already cascades. Resolution order is per-validator `Severity` > category `Severity` > the validator's built-in default > the global `warning` default, so a per-validator setting still wins. Previously a category-level `Severity` was silently ignored (it validated but did nothing). This is backwards compatible: every shipped validator sets its `Severity` explicitly, so default behaviour is unchanged, and configs without a category-level `Severity` resolve exactly as before - only a category `Severity` key that used to be a no-op now takes effect. Invalid category severities are now rejected with the same message as per-validator ones.
