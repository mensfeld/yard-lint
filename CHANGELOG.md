...
- **[Bugfix]** `Documentation/BlankLineBeforeDefinition` no longer flags an undocumented definition that follows a line of commented-out code separated by a blank line (e.g. a constant block with some entries commented out: `# KEY_F12 = ...` then a blank line then `KEY_C_A = ...`). Such a comment is code, not a docstring detached from the next definition, so the blank line is an intentional separator rather than an orphaned-documentation offense. Commented-out assignments and `def`/`class`/`module`/`require`/mixin/`attr_*` statements are now recognized as non-documentation, alongside the magic comments, shebangs, and linter directives already skipped. Genuine detached docstrings (real prose comments orphaned by a blank line) are still reported (#300).
