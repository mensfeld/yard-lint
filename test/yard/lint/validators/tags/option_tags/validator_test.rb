...
class YardLintValidatorsTagsOptionTagsValidatorTest < Minitest::Test
  attr_reader :config, :selection, :validator

  def setup
    @config = Yard::Lint::Config.new
    @selection = ['lib/example.rb']
    @validator = Yard::Lint::Validators::Tags::OptionTags::Validator.new(config, selection)
  end
  ...
  def test_in_process_returns_true_for_in_process_execution
    assert_equal(true, Yard::Lint::Validators::Tags::OptionTags::Validator.in_process?)
  end
end
