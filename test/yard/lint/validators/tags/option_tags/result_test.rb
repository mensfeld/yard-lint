...
class YardLintValidatorsTagsOptionTagsResultTest < Minitest::Test
  attr_reader :config, :parsed_data, :result

  def setup
    @config = Yard::Lint::Config.new
    @parsed_data = []
    @result = Yard::Lint::Validators::Tags::OptionTags::Result.new(@parsed_data, @config)
  end
  ...
  def test_class_methods_defines_default_severity
    assert_respond_to(Yard::Lint::Validators::Tags::OptionTags::Result, :default_severity)
  end

  def test_class_methods_defines_offense_type
    assert_respond_to(Yard::Lint::Validators::Tags::OptionTags::Result, :offense_type)
  end

  def test_class_methods_defines_offense_name
    assert_respond_to(Yard::Lint::Validators::Tags::OptionTags::Result, :offense_name)
  end
end
